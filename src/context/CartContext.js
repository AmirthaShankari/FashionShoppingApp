...
const cartReducer = (state, action) => {
    switch (action.type) {
        case C.ADD_TO_CART:
            ...
            AsyncStorage.setItem(AppConstants.ASYNC_STORAGE.CART, JSON.stringify(newCart));
...
const getStoredCart = (dispatch) => async () => {
    try {
        let storedCart = await AsyncStorage.getItem(AppConstants.ASYNC_STORAGE.CART);
        log.info('Fetching the stored cart! stored cart...', storedCart)
        if (!storedCart) {
            storedCart = [];
            await AsyncStorage.setItem(AppConstants.ASYNC_STORAGE.CART, JSON.stringify(storedCart));
